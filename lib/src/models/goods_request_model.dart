...
//
//     final goodsRequestModel = goodsRequestModelFromJson(jsonString);

import 'dart:convert';

List<GoodsRequestModel> goodsRequestModelFromJson(String str) => List<GoodsRequestModel>.from(json.decode(str).map((x) => GoodsRequestModel.fromJson(x)));

String goodsRequestModelToJson(List<GoodsRequestModel> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
class GoodsRequestModel {
    ...
    factory GoodsRequestModel.fromJson(Map<String, dynamic> json) => GoodsRequestModel(
        id: json["id"],
        userId: json["user_id"],
